        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, i) in spuSaleAttrList" :key="item.id">
                <dt class="title">选择{{ item.saleAttrName }}</dt>
                <dd
                  @click="changeAttr(item.spuSaleAttrValueList, i)"
                  changepirce="0"
                  :class="{ active: value.isChecked == 1 ? true : false }"
                  v-for="(value, i) in item.spuSaleAttrValueList"
                  :key="value.id"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  v-model.number="num"
                />
                <a @click="numincrement" href="javascript:" class="plus">+</a>
                <a @click="numdecrement" href="javascript:" class="mins">-</a>
              </div>
              <div class="add">
                <a @click="submitToshopcart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      num: 1,
      uuid_token: localStorage.getItem("USER_ID")
    };
  },
  components: {
    ImageList,
  ...
  mounted() {
    ...
    this.$store.dispatch("detail/getinfo", this.$route.params.skuId);
    // console.log(this.skuAttrValueList, "这是skuAttrValueList");
  },
  methods: {
    ...
    async submitToshopcart() {
      try {
        console.log(this.skuInfo.id, this.num);
        await this.$store.dispatch("detail/addshopcart", {
          skuId: this.skuInfo.id,
          num: this.num,
        });
        //  出错我感觉是因为 router-link直接跳走了，上面那个alert都没出来
